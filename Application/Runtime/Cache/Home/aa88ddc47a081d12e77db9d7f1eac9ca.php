                ...
                <form action="<?php echo U();?>" method="post" id="subForm">
                    ...
                    <ul>
                        ...
                        <li>
                            ...
                            <span class="fill">
                                <?php if(!empty($data["idcard_img"])): ?><img src="<?php echo ($data["idcard_img"]); ?>" id="img" style="width:300px;height:150px;border:1px solid #eee;">
                                <?php else: ?>
                                    <input type="button" id="upload-btn" value="上传">
                                    <div class="img-box"></div>
                                    <input type="hidden" name="idcard_img" id="idcard_img" value="<?php echo ($data["idcard_img"]); ?>"><?php endif; ?>
							</span>
                        </li>
                    </ul>
                    <?php if(($data["bind_bank"]) == "0"): ?><p><input type="submit" id="sub-btn" post-form="#subForm" value="保存"></p><?php endif; ?>
                </form>
            </div>
        </div>
    </div>
</div>
<script src="/Public/home/js/jquery.upload.original.js"></script>
<script>
    ...
    $('#upload-btn').click(function () {
        ...
        $.upload({
            url: '<?php echo U("File/ajaxUpload?type=IDCARD");?>',
            accept : 'image',
            dataType: 'json',
            onSend: function () {
                return true;
